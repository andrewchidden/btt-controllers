#!/bin/bash

#####
# DESCRIPTION: Updates the appearance of the volume control icon by reading
# the contents of the service-owned status file.
#
# USAGE: ./volume-controller
#     <sys_root>
#         Path to the BetterTouchTool system service and caches folder.
#####


##### @interface #####
# Optionally set the `BTT_SYS_ROOT` environment variable to the internal system 
# folder used by services to save and share state. Defaults to `~/.btt`
sys_root_arg=${1:-${BTT_SYS_ROOT}}
btt_sys_root=${sys_root_arg:-~/.btt}


##### @implementation #####
status_directory="${btt_sys_root}/volume-service"
status_filepath="${status_directory}/status"

output_volume="$(cat $status_filepath)"
if [[ -n "${output_volume}" ]]; then
	if [[ "${output_volume}" -gt 65 ]]; then # 3 bars
		icon_data=''
	elif [[ "${output_volume}" -gt 32 ]]; then # 2 bars
		icon_data=''
	elif [[ "${output_volume}" -gt 0 ]]; then # 1 bar
		icon_data=''
	elif [[ "${output_volume}" = "-0" ]]; then # muted 0 bars
		icon_data=''
	elif [[ "${output_volume}" = 0 ]]; then # 0 bars
		icon_data=''
	elif [[ "${output_volume}" -gt -33 ]]; then # muted 1 bar
		icon_data=''
	elif [[ "${output_volume}" -gt -66 ]]; then # muted 2 bars
		icon_data=''
	else # muted 3 bars
		icon_data=''
	fi
else
	icon_data=''
fi

echo "{\"text\":\"volume\", \"icon_data\": \"${icon_data}\"}"